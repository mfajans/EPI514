...
#bfss2015_2019$age_grp, brfss2015_2019$age_grp2, useNA = "always", deparse.level=2)
    #agr groups recategorized in earlier script
...
novaxprev_cismen<-svytable(~nofluvax_bin + equality, cismen_prev2b) 
prop.table(novaxprev_cismen, margin=2)*100

#PR
reg2b_cismen<-svyglm(nofluvax_bin~equality, design=cismen_prev2b, family = quasibinomial(link = "log"))
reg2b_cismen

exp(coef(reg2b_cismen))
exp(confint(reg2b_cismen))
...
novaxprev_ciswomen<-svytable(~nofluvax_bin + equality, ciswomen_prev2b) 
prop.table(novaxprev_ciswomen, margin=2)*100

#PR
reg2b_ciswomen<-svyglm(nofluvax_bin~equality, design=ciswomen_prev2b, family = quasibinomial(link = "log"))
reg2b_ciswomen

exp(coef(reg2b_ciswomen))
exp(confint(reg2b_ciswomen))
...
novaxprev_genpoltab<-svytable(~nofluvax_bin + equality + gender_re, brfss_svy)

novaxprev_genpoltab #Hand-checked proportions, weighted prevalences are identical

#9/18 (CJF): These proportions no longer match using gender_re
...
age2 <- svyglm(nofluvax_bin ~ gender_re, 
               design = subset(brfss_svy, age_grp == "25-44"),
               family = quasibinomial(link = "log"))

age3 <- svyglm(nofluvax_bin ~ gender_re, 
               design = subset(brfss_svy, age_grp == "45-64"),
               family = quasibinomial(link = "log"))
...
svytable(~nofluvax_bin + gender_re + equality, brfss_svy) %>%
  prop.table(margin = 2)

equality_p <- svyglm(nofluvax_bin ~ gender_re, 
                         design = subset(brfss_svy, equality == "Protective"),
                         family = quasibinomial(link = "log"))

equality_r <- svyglm(nofluvax_bin ~ gender_re, 
                            design = subset(brfss_svy, equality == "Restrictive"),
                            family = quasibinomial(link = "log"))

exp(coef(equality_p))
exp(coef(equality_r))

exp(confint(equality_p))
exp(confint(equality_r))


#CJF (9/19): Potentially remove this section from the script?
...
pd2b_ciswomen 
pd2b_ciswomen %>% coef %>% round(3)
pd2b_ciswomen %>% confint %>% round(3)    

#PD cisgender men
pd2b_cismen <-svyglm(nofluvax_bin~equality, design=cismen_prev2b)
pd2b_cismen 
pd2b_cismen %>% coef %>% round(3)
pd2b_cismen %>% confint %>% round(3) 
