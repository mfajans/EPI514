...
#Supress where n <= 15 (for TGNB)

#Create subset of states we want
tm_state_sub15<-c("Connecticut",
                  "Delaware",
                  "Florida",
                  "Hawaii",
                  "Indiana",
...
brfss_tm_sub <- brfss_tm %>% filter(state %in% tm_state_sub15)
brfss_tf_sub <- brfss_tf %>% filter(state %in% tf_state_sub15)
brfss_nb_sub <- brfss_nb %>% filter(state %in% nb_state_sub15)

#Create new table subsets  >=15
rrTab2c_tm_sub <- table(brfss_tm_sub$state, brfss_tm_sub$nofluvax_re,
                        deparse.level = 2)
rrTab2c_tf_sub <- table(brfss_tf_sub$state, brfss_tf_sub$nofluvax_re,
                        deparse.level = 2)
rrTab2c_nb_sub <- table(brfss_nb_sub$state, brfss_nb_sub$nofluvax_re,
                     deparse.level = 2)
...
prop.table(rrTab2c_tm_sub, 1)
prop.table(rrTab2c_tf_sub, 1)
prop.table(rrTab2c_nb_sub, 1)
...
map_nb_sub<-as.data.frame(prop.table(rrTab2c_nb_sub, 1))
map_nb_sub<-map_nb_sub %>% dplyr::rename(state = brfss_nb_sub.state,
                                 unvaccinated = Freq)
map_nb_sub<-map_nb_sub %>% mutate(unvaccinated_per = paste(round(unvaccinated*100,0), "%", sep= ""))
map_nb_sub<-map_nb_sub %>% mutate(unvaccinated_cat = case_when((unvaccinated > 0 & unvaccinated <= 0.1) ~ 1, 
                                                       (unvaccinated > 0.1 & unvaccinated <= 0.2) ~ 2, 
                                                       (unvaccinated > 0.2 & unvaccinated <= 0.3) ~ 3,
                                                       (unvaccinated > 0.3 & unvaccinated <= 0.4) ~ 4,
                                                       (unvaccinated > 0.4 & unvaccinated <= 0.5) ~ 5,
...
map_tm_sub<-as.data.frame(prop.table(rrTab2c_tm_sub, 1))
map_tm_sub<-map_tm_sub %>% dplyr::rename(state = brfss_tm_sub.state,
                                         unvaccinated = Freq)
map_tm_sub<-map_tm_sub %>% mutate(unvaccinated_per = paste(round(unvaccinated*100,0), "%", sep= ""))
map_tm_sub<-map_tm_sub %>% mutate(unvaccinated_cat = case_when((unvaccinated > 0 & unvaccinated <= 0.1) ~ 1, 
                                                               (unvaccinated > 0.1 & unvaccinated <= 0.2) ~ 2, 
                                                               (unvaccinated > 0.2 & unvaccinated <= 0.3) ~ 3,
                                                               (unvaccinated > 0.3 & unvaccinated <= 0.4) ~ 4,
                                                               (unvaccinated > 0.4 & unvaccinated <= 0.5) ~ 5,
...
#Transgender Female Supressed Subset
map_tf_sub<-as.data.frame(prop.table(rrTab2c_tf_sub, 1))
map_tf_sub<-map_tf_sub %>% dplyr::rename(state = brfss_tf_sub.state,
                                         unvaccinated = Freq)
map_tf_sub<-map_tf_sub %>% mutate(unvaccinated_per = paste(round(unvaccinated*100,0), "%", sep= ""))
map_tf_sub<-map_tf_sub %>% mutate(unvaccinated_cat = case_when((unvaccinated > 0 & unvaccinated <= 0.1) ~ 1, 
                                                               (unvaccinated > 0.1 & unvaccinated <= 0.2) ~ 2, 
                                                               (unvaccinated > 0.2 & unvaccinated <= 0.3) ~ 3,
                                                               (unvaccinated > 0.3 & unvaccinated <= 0.4) ~ 4,
                                                               (unvaccinated > 0.4 & unvaccinated <= 0.5) ~ 5,
...
#2b. By Gender Supressed subset
figure2b_sub<-  map_cis_plot + map_nb_sub_plot +  map_tf_sub_plot + map_tm_sub_plot +
  plot_layout(ncol = 2)
figure2b_sub  
