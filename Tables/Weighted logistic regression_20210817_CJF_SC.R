...
#save survey cismen object 
write_rds(brfss_svy_cismen, paste0(dataDir, "brfss_svy_cismen.rds")) #save
...
#save survey object 
write_rds(brfss_svy, paste0(dataDir, "brfss_svy.rds")) #save
...
#save survey cismen object 
write_rds(brfss_svy_ciswomen, paste0(dataDir, "brfss_svy_ciswomen.rds")) #save
...
#Prevalence Protective vs. Restrictive
...
#Prevalence by State
novaxprev_cis_state<-svytable(~nofluvax_bin + state, cis_prev2b) 
test <- prop.table(novaxprev_cis_state, margin=2)*100

write.csv(test, "/Users/sarahcox/Desktop/PhD/2021 Spring/S21_EPI514_ApplicationEpiMethods/data/test.csv", row.names = TRUE) 

prop.table(novaxprev_cis_state)*100

table(novaxprev_cis_state$state, novaxprev_cis_state$nofluvax_bin)
tab_cis<-prop.table(novaxprev_cis_state, margin=2)*100
df_cis <- data.frame(novaxprev_cis_state=names(tab_cis), prevalence=as.numeric(tab_cis))

novaxprev_cis_state2<-svytable(~state + nofluvax_bin, cis_prev2b) 
prop.table(novaxprev_cis_state2, margin=2)*100
...
svytable(~age_grp + nofluvax_bin, brfss_svy) %>% prop.table(margin=1) #looks like age might be acting more as an effect modifier
svytable(~age_grp + gender_re, brfss_svy) %>% prop.table(margin=1) #Same

age_nofluvax<-svyglm(nofluvax_bin~age_grp, brfss_svy, family=quasibinomial(link = "log"))
...
svytable(~nofluvax_bin + gender_re + age_grp, brfss_svy) %>%
  prop.table(margin = 2)
...
svytable(~age_grp + equality, brfss_svy) %>% prop.table(margin=1) #Looks pretty similar
svytable(~age_grp + nofluvax_bin, brfss_svy) %>% prop.table(margin=1) #Looks like a strong association, but again could be an effect modifier
...
age_equality<-svyglm(equality~age_grp, brfss_svy, family=quasibinomial(link = "log"))
...
#Cisgender
svytable(~nofluvax_bin +equality + age_grp, brfss_svy) %>%
  prop.table(margin = 2)

age1_cis <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "18-24" & gender_re2=="Cisgender"),
                   family = quasibinomial(link = "log"))

age2_cis <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "25-34"& gender_re2=="Cisgender"),
                   family = quasibinomial(link = "log"))

age3_cis <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "35-44"& gender_re2=="Cisgender"),
                   family = quasibinomial(link = "log"))

age4_cis<- svyglm(nofluvax_bin ~ equality,
                  design = subset(brfss_svy, age_grp == "45-54"& gender_re2=="Cisgender"),
                  family = quasibinomial(link = "log"))

age5_cis <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "55-64"& gender_re2=="Cisgender"),
                   family = quasibinomial(link = "log"))
...
age1_tgw <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "18-24" & gender_re2=="Transgender Women"),
                   family = quasibinomial(link = "log"))

age2_tgw <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "25-34"& gender_re2=="Transgender Women"),
                   family = quasibinomial(link = "log"))

age3_tgw <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "35-44"& gender_re2=="Transgender Women"),
                   family = quasibinomial(link = "log"))

age4_tgw<- svyglm(nofluvax_bin ~ equality,
                  design = subset(brfss_svy, age_grp == "45-54"& gender_re2=="Transgender Women"),
                  family = quasibinomial(link = "log"))

age5_tgw <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "55-64"& gender_re2=="Transgender Women"),
                   family = quasibinomial(link = "log"))
...
age1_tgm <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "18-24" & gender_re2=="Transgender Men"),
                   family = quasibinomial(link = "log"))

age2_tgm <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "25-34"& gender_re2=="Transgender Men"),
                   family = quasibinomial(link = "log"))

age3_tgm <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "35-44"& gender_re2=="Transgender Men"),
                   family = quasibinomial(link = "log"))

age4_tgm<- svyglm(nofluvax_bin ~ equality,
                  design = subset(brfss_svy, age_grp == "45-54"& gender_re2=="Transgender Men"),
                  family = quasibinomial(link = "log"))

age5_tgm <- svyglm(nofluvax_bin ~ equality, 
                   design = subset(brfss_svy, age_grp == "55-64"& gender_re2=="Transgender Men"),
                   family = quasibinomial(link = "log"))
...
age1_nb <- svyglm(nofluvax_bin ~ equality, 
                  design = subset(brfss_svy, age_grp == "18-24" & gender_re2=="Non-binary"),
                  family = quasibinomial(link = "log"))

age2_nb <- svyglm(nofluvax_bin ~ equality, 
                  design = subset(brfss_svy, age_grp == "25-34"& gender_re2=="Non-binary"),
                  family = quasibinomial(link = "log"))

age3_nb <- svyglm(nofluvax_bin ~ equality, 
                  design = subset(brfss_svy, age_grp == "35-44"& gender_re2=="Non-binary"),
                  family = quasibinomial(link = "log"))

age4_nb<- svyglm(nofluvax_bin ~ equality,
                 design = subset(brfss_svy, age_grp == "45-54"& gender_re2=="Non-binary"),
                 family = quasibinomial(link = "log"))

age5_nb <- svyglm(nofluvax_bin ~ equality, 
                  design = subset(brfss_svy, age_grp == "55-64"& gender_re2=="Non-binary"),
                  family = quasibinomial(link = "log"))
