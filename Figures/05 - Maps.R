...
brfss2015_2019 <- readRDS("~/Mark's Work/UW/Classes/Spring 2021/EPI 514 - Applications of Epi/Data/Databrfss2015_2019.rds") #check
brfss_t <- readRDS("~/Mark's Work/UW/Classes/Spring 2021/EPI 514 - Applications of Epi/Data/Databrfss_t.rds") #check
################################################################################
...
###################################################
#Mark's code for restrictive vs protective states
###################################################

#Pulling in restrictive vs protective state csv
state<-read.csv("statepolicy.csv")
state$fips<-fips(state$ï..State)

table(state$brfssid, state$fips )

state$equality_cat<-factor(state$equality,
                           levels = c(1,2),
                           labels = c("Restrictive",
                                      "Protective"))


plot_usmap( data=state, values="equality_cat", 
            color="black")+
        scale_fill_manual(values=c("#FC4E07", "#00AFBB"), name="State Policy on Gender Identity")+
        theme(legend.position = "right")
