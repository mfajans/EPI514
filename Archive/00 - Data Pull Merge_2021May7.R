...
brfss <- brfss %>% mutate(birthsex = case_when(SEX == 1 ~ 1, #Male
                                               ...
                                               SEX == 9 ~ 9)) #Refused
#Code Missing to show up in table since >5%
brfss$birthsex[is.na(brfss$birthsex)] <- 999
brfss$birthsex  <-  factor(brfss$birthsex, 
                           levels = c(1, 2, 7, 9, 999), 
                           labels=c("Male", 
                                    "Female",
                                    "Don't Know/ not sure",
                                    "Refused",
...
brfss<-brfss %>% mutate(health_coverage = case_when(HLTHPLN1==1 ~ 1, #Yes
                                                    HLTHPLN1==2 ~ 2, #No
                                                    ))        
brfss$health_coverage[brfss$HLTHPLN1==7]<-NA
brfss$health_coverage[brfss$HLTHPLN1==9]<-NA

brfss$health_coverage  <-  factor(brfss$health_coverage, 
                        levels = c(1,2),
                        labels = c("Yes",
                                   "No"))

table(brfss$HLTHPLN1,brfss$health_coverage, useNA="always")

table(brfss$health_coverage, useNA="always")
...
brfss<-brfss %>% mutate(medcost = case_when(MEDCOST==1 ~ 1, #Yes
                                            MEDCOST==2 ~ 2, #No
                                            )) 
...
table(brfss$MEDCOST, brfss$medcost, useNA = "always")

brfss$medcost  <-  factor(brfss$medcost, 
                                  levels = c(1,2),
...
brfss$employ_recat[brfss$EMPLOY1==9]<-NA
...
table(brfss$employ_recat, brfss$EMPLOY1, useNA="always")
...
brfss<-brfss %>% mutate(mentalhlth = case_when((MENTHLTH>=14 & MENTHLTH<=30) ~ 1, #>=14 days 
                                               (MENTHLTH<14 | MENTHLTH==88)  ~ 2 #<14 days
                                                 ))

brfss$mentalhlth[brfss$MENTHLTH==77]<-NA                                                 
brfss$mentalhlth[brfss$MENTHLTH==99]<-NA



brfss$mentalhlth <-  factor(brfss$mentalhlth, 
                              levels = c(1,2),
                              labels = c(">=14 days",
                                         "<14 days"))
                                         

table(brfss$MENTHLTH, brfss$mentalhlth, useNA="always")
table(brfss$mentalhlth, useNA="always")
...
brfss$race[brfss$race=="Don't know/Not sure/Refused"] <- NA

table(brfss$race, useNA = "always") #check
...
table1::label(brfss$race) <- "Race/ethnicity"
...
table1::label(brfss$employ_recat) <-"Employment Status"
table1::label(brfss$health_coverage) <-"Health Insurance"
table1::label(brfss$medcost_re) <-"Poor Health Access (Could not see a doctor because of cost)"
table1::label(brfss$mentalhlth) <-"Poor Mental Health Status"
table1::label(brfss$dataYear_cat) <-"BRFSS Year"
#*******continue********#

## Table 1a
table1a <- table1(~ birthsex
                 + sexorient
                 + age_grp
                 + race 
                 + bmi_cat
